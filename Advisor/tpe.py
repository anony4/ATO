...
class TPEAdvisor(Baseline):
    def __init__(
            ...
            warm_start_flag=True, backup_flag=False


    ):
        super().__init__(config_space, eval_func, iter_num, save_dir, method_id=method_id, task_id=task_id,
                         target=target,
                         context_flag=False,
                         tsd_flag='no_detect',
    ...
    def fit_kde_models(self):
        num_config_successful = len(self.observations)
        ...
        train_configs = convert_configurations_to_array(self.configurations)
        train_losses = np.asarray(self.objectives, dtype=np.float64).reshape(-1)
    ...
    def sample_random_configs(self, num_configs=1):
        ...
        while len(configs) < num_configs:
            ...
            sample_cnt += 1
            if config not in (self.configurations + configs):
